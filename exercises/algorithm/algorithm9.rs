    ...
    T: Default,
{
    ...
    fn push_up(&mut self, mut idx: usize) {
    ...
    fn push_down(&mut self, mut idx: usize) {
        while self.children_present(idx) {
            let smaller = self.smallest_child_idx(idx);

            if (self.comparator)(&self.items[smaller], &self.items[idx]) {
                self.items.swap(smaller, idx);
                idx = smaller;
            } else {
                break;
            }
